...
- **Don't give up on an adapter after one failed attempt.** Some sites have multiple adapters under different names (e.g. `rednote` vs `xiaohongshu` — both exist but `xiaohongshu search` works while `rednote search` returned empty). If one adapter name fails, run `opencli list | grep -i <site>` to check for alternative names before falling back to browser tools.
- **The `note` and `comments` commands need full signed URLs, not bare IDs.** `opencli xiaohongshu note <id>` will fail with ARGUMENT error. Get the full `url` (including `xsec_token`) from search results and pass that instead.
