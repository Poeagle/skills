...
- **Don't give up on an adapter after one failed attempt.** Some sites have multiple adapters under different names (e.g. `rednote` vs `xiaohongshu` — both exist but `xiaohongshu search` works while `rednote search` returns `AUTH_REQUIRED` or empty). If one adapter name fails — regardless of the error code (AUTH_REQUIRED, empty results, connection refused) — run `opencli list | grep -i <site>` to check for alternative names before falling back to browser tools. The user's Chrome session may be logged into one domain but not the other.
- **The `note` and `comments` commands need full signed URLs, not bare IDs.** `opencli xiaohongshu note <id>` will fail with ARGUMENT error and a `help` message telling you to use the signed URL. Get the full `url` (including `xsec_token`) from the search results JSON you already have (`url` field in each result) and pass that instead. **Do NOT interpret the ARGUMENT error as 'blocked by anti-bot' or 'site unavailable' — the error message tells you exactly what to do, and the fix is already in your search results.**
