...
11. **ALWAYS close/unbind sessions when done.** Every browser session leaves a tab open in the user's Chrome until explicitly released. This is a first-class UX issue — leftover tabs interfere with normal browsing and force the user to manually clean up. Pattern: after the last data extraction, immediately run `opencli browser <session> unbind` (for bound sessions) or `opencli browser <session> close` (for owned sessions). For adapter commands, use `--site-session ephemeral` so the session auto-closes. Never leave a session dangling between tool calls.
...
### Extract structured list data via eval (YouTube, Twitter, etc.)

When `state` output is too large (100K+ chars on complex SPAs), use `eval` with a targeted JS IIFE.
Full YouTube selectors and extraction patterns: see `references/youtube-dom-extraction.md`.

```bash
# Step 1: user opens YouTube history in Chrome
opencli browser yt bind
# Step 2: eval targeted JS
opencli browser yt eval '(() => {
  const results = [];
  document.querySelectorAll("ytd-item-section-renderer").forEach(sec => {
    const date = sec.querySelector("h2#title")?.textContent?.trim() || "";
    sec.querySelectorAll("yt-lockup-view-model").forEach(v => {
      ...
      if (title) results.push({date, title, channel});
    });
  });
  return JSON.stringify(results, null, 2);
})()'
# Step 3: unbind
opencli browser yt unbind
```

This pattern works for any SPA with complex custom elements — find the right query selectors once, then extract clean JSON in one call.

### Extract video content via subtitles/transcripts

When you need to know the *content* of a video (not just metadata), use the platform's subtitle/transcript API rather than watching the video. This works for Bilibili, YouTube, and others that have subtitle/adaptation track endpoints.

```bash
# Bilibili — full subtitle extraction
...
# YouTube — use youtube-content skill or transcript endpoint
opencli youtube transcript <url> -f json
```

Key points:
- Requires the video to have subtitles (user-uploaded or auto-generated). If empty, the video has no subtitle track.
- Output is a flat list of `{from, to, content}` entries. Pipe through python3 to get clean text.
- Combine with `opencli <site> video <id>` for metadata (title, author, views, description).
- This is the standard pattern for "what does this video say?" questions — no need to watch or listen.
- Works for 22-min videos (full transcript ~800 entries, ~80KB JSON). For longer videos, consider sampling key segments.

### Multi-platform video research

When researching a topic across multiple video platforms (YouTube, Bilibili, Xiaohongshu, etc.):

1. Search each platform with `opencli <site> search "<query>" --window background --site-session ephemeral -f json`
2. Identify the most relevant/authoritative videos from each platform
3. Extract subtitles from the top 1-2 videos per platform: `opencli <site> subtitle <id>`
4. Synthesize findings across platforms — each platform has a different content culture (B站 = deep dives, YouTube = vlogs, 小红书 = lifestyle tips)
5. Always include a search summary at the end listing platforms, queries, and call counts

```bash
# Parallel search across 3 platforms
opencli youtube search "数字极简" --window background --site-session ephemeral --limit 10 -f json &
opencli bilibili search "数字极简" --window background --site-session ephemeral --limit 10 -f json &
opencli xiaohongshu search "数字极简" --window background --site-session ephemeral --limit 10 -f json &
wait
```
...
- **`--window background` only prevents focus-stealing, not tab creation.** The flag stops OpenCLI from bringing the browser window to the foreground, but it still creates a new tab. Users expecting "invisible" operation will be surprised by leftover tabs. Use `--site-session ephemeral` to auto-close after the command, or explicitly close sessions after use.
...
- **COOKIE-strategy adapter commands fail when the user isn't logged in.** `opencli youtube history`, `opencli facebook notifications`, etc. open their own browser window which won't have the user's session. If the adapter returns a login/empty error, fall back to: (1) ask user to open the target page in their Chrome, (2) `opencli browser <session> bind` to that tab, (3) use `eval` or `state` to extract data. This is the standard auth fallback pattern.
- **`state` output on complex SPAs (YouTube, Twitter, etc.) can exceed 100K chars.** Avoid parsing `state` for structured list data. Instead, use `eval` with a targeted JS IIFE that queries specific DOM selectors and returns clean JSON. One `eval` call replaces hours of parsing `state` output.
...
- `references/youtube-dom-extraction.md` — YouTube DOM selectors for eval-based extraction
- `references/chinese-market-data.md` — A-share indices, ETFs, gold, and crypto via Tencent Finance API and CoinGecko (fallback when no opencli finance adapter exists)
- `references/weibo-chat-browser.md` — Weibo chat/group messages via browser driving (no adapter exists)
- `references/xiaohongshu-extraction.md` — Xiaohongshu post extraction: two-step search→note pattern, signed URL requirements, pitfalls
- `references/chrome-local-history.md` — (in smart-search) — Reading Chrome's local SQLite history for cross-platform behavioral analysis
