...
5. **项目体量预检**：在开始深入源码前，先做复杂度评估：
   ```
   检测源码文件数和总代码行数：
   - 小型项目（< 50 文件 或 < 5000 行）：按标准流程完整解构
   - 大型项目（>= 50 文件 或 >= 10000 行）：标记为"大项目"
     - 优先产出顶层文档（README、架构概览、通信图、跨切面概念）
     - 组件详情标注为"首次分析"，内容深度适当但不承诺完整覆盖
     - 在 log.md 中记录未覆盖的组件，供后续补充
     - 向用户说明大项目分析范围
   ```
