...
5. **触发 code-design skill**：按 `code-design` 技能的逆向分析（Analyze）模式执行完整解构流程。**不要使用 Agent 工具并行探索**，必须串行依次完成全局概览和每个组件详情。
...
7. **记录本地路径到 README.md**：读取 `raw/05-coderepo/{文件名}` 的当前内容（步骤 3 已改为本地路径 `~/sources/{仓库名}`），将其作为本地仓库地址写入 `wiki/code-design/{仓库名}/README.md`，与远程地址并列展示：
   ```markdown
   ## 仓库地址
...
8. **更新 log.md**：追加操作日志。
8.5. **质量自检**：在等待用户确认前，必须先做自我检查：
   - 用 `grep` 检查是否有 `[[` 双链残留（**code-design 文档禁止使用任何 wikilink，一律用纯文本引用**）、` ```mermaid ` 残留
   - 用 `find` 确认没有空组件目录
   - 审查 Read 工具调用记录，确认每个组件的写作都有对应的源码阅读操作
   - **运行 lint 脚本验证**：`python3 ~/.claude/skills/lint/scripts/lint.py`，检查 `code_design` 字段中该仓库的 `passed/total_components` 是否等于 1（全部通过）。如有 `failed > 0`，逐条修复后再继续
   - 发现任何问题立即**删除重写**，不允许带着问题交差
9. **等待用户确认**：向用户报告本次代码解构的产出物概览（多少组件、多少文件、覆盖了哪些源码目录），询问用户质量是否达标。用户确认通过后才执行下一步。未通过时根据反馈修改或补充。
10. **清理标志文件**：用户确认通过后，删除 `raw/05-coderepo/{文件名}`，表示该仓库已处理完成。
