...
description: 仓库治理审计、初始化与代码生成约束。审计现有仓库治理健康度，按 openclaw Agent-first Governance 模式初始化新仓库，并在已治理仓库中约束 agent 代码生成行为。当用户提到"检查治理"、"审计仓库"、"初始化仓库"、"创建新项目"时使用。
---
...
| 模式 | 触发词 | 说明 |
|------|--------|------|
| **Init** | `初始化仓库`, `创建新项目`, `/sc init` | 交互式创建完整治理结构 |
| **Audit** | `检查治理`, `审计仓库`, `/sc audit` | 深度检测仓库治理健康度 |
