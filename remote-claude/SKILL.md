...
# 一行检查全部（curl 版本，适合人类）
echo "=== 健康检查 ==="
curl -s http://localhost:42010/api/health 2>&1 | python3 -m json.tool

echo "=== 项目 + skip_permissions ==="
curl -s http://localhost:42010/api/projects 2>&1 | python3 -c "import json,sys; d=json.load(sys.stdin); [print(f\"  {p['name']}: {'✅' if p.get('skip_permissions') else '⚠️  skip_permissions=0'}\") for p in d['projects']]"

echo "=== session_claude_command ==="
curl -s http://localhost:42010/api/settings 2>&1 | python3 -c "import json,sys; print(json.load(sys.stdin)['settings'].get('session_claude_command','❌ NOT SET'))"

echo "=== 已有 sessions（按状态分组）==="
curl -s http://localhost:42010/api/sessions 2>&1 | python3 -c "
import json,sys
d=json.load(sys.stdin)['sessions']
from collections import Counter
for k,v in Counter(s['status'] for s in d).items():
    print(f'  {k}: {v}个')
running=[s for s in d if s['status']=='running']
pending=[s for s in d if s['status']=='pending']
if pending: print(f'  ⚠️  有 {len(pending)} 个 pending session 未清理')
if running: print(f'  ✅ 有 running session: {running[0][\"id\"][:12]}..., 可直接复用')
"

echo "=== 清理积累的 pending/failed session ==="
# 清理所有 pending 和 failed 的 session（保留 running）
curl -s http://localhost:42010/api/sessions 2>&1 | python3 -c "
import json, requests, sys
d=json.load(sys.stdin)['sessions']
stale=[s for s in d if s['status'] in ('pending','failed','cancelled')]
BASE='http://localhost:42010/api'
for s in stale:
    requests.delete(f\"{BASE}/sessions/{s['id']}\")
    print(f'  deleted {s[\"status\"]}: {s[\"id\"][:12]}...')
print(f'  清理了 {len(stale)} 个 stale session')
print(f'  剩余 running: {len([s for s in d if s[\"status\"]==\"running\"])}个')
"
```
...
print(f"OctoAlly: {h['status']} (v{h['version']})")

# 2. 项目 + skip_permissions
projects = requests.get(f"{BASE}/projects").json()
for p in projects["projects"]:
    ok = p.get("skip_permissions")
    print(f"  {p['name']}: {'✅' if ok else '⚠️  skip_permissions=0'}")

# 3. 自动修复 skip_permissions
for p in projects["projects"]:
    if not p.get("skip_permissions"):
        requests.patch(f"{BASE}/projects/{p['id']}", json={"skip_permissions": 1})
        print(f"  → 已修复 {p['name']}")

# 4. 检查 session_claude_command
s = requests.get(f"{BASE}/settings").json()["settings"]
print(f"  session命令: {s.get('session_claude_command', '❌ 未设置')}")

# 5. 清理 stale sessions + 输出可用 session
sessions = requests.get(f"{BASE}/sessions").json()
for sess in sessions["sessions"]:
    if sess["status"] in ("pending", "failed", "cancelled"):
        requests.delete(f"{BASE}/sessions/{sess['id']}")

running = [s for s in sessions["sessions"] if s["status"] == "running"]
if running:
    print(f"  ✅ 可复用 running session: {running[0]['id'][:12]}...")
```
...
> **可能只是暂时的：** 如果已经有 running session，OctoAlly 可能需要 ~10s 才能将新 session 转为 running。轮询等待 15s 看是否自动恢复。

**排查顺序（按频率排序）：**

### 1. OctoAlly worker 池被占用
当前只有一个 worker，如果有 running session 占着，新 session 排队。

**检查：** `GET /api/sessions` 看是否有 `running` 状态。
**修复：** 等待 10-15s 看是否自动转为 running。
**强制清理：** `DELETE /api/sessions/:id` 杀掉不用 running session。
**Workaround：** 直接对已有 running session 发 execute，不用新建。

### 2. skip_permissions = 0
项目配置未开启权限跳过，claude 启动时卡在授权确认。

**检查：** `GET /api/projects` 看该项目的 `skip_permissions` 字段。
**修复：** `PATCH /api/projects/:id {"skip_permissions": 1}`

### 3. claude CLI 是 shell 别名
