...
本文档定义了 skill-creator 所使用的 JSON 模式。
...
- `skill_name`：被改进的技能的名称
- `current_best`：表现最佳的版本的标识符
...
一次运行的挂钟计时数据。位于 `<run-dir>/timing.json`。

**如何捕获：** 当子代理任务完成时，任务通知中包含 `total_tokens` 和 `duration_ms`。请立即保存这些数据——它们不会在其他地方持久化，事后无法恢复。
...
- `run_summary`：按配置统计的聚合数据
  - `with_skill` / `without_skill`：各自包含 `pass_rate`、`time_seconds`、`tokens` 对象，带有 `mean` 和 `stddev` 字段
  - `delta`：差值字符串，如 `"+0.50"`、`"+13.0"`、`"+1700"`
- `notes`：分析器的自由格式观察笔记

**重要提示：** 查看器按字段名精确读取这些内容。使用 `config` 代替 `configuration`，或将 `pass_rate` 放在运行对象的顶层而非嵌套在 `result` 中，会导致查看器显示空值/零值。手动生成 benchmark.json 时请始终参考此模式。

---

## comparison.json

盲比器的输出。位于 `<grading-dir>/comparison-N.json`。
