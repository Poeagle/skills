...
def generate_html(data: dict, auto_refresh: bool = False, skill_name: str = "") -> str:
    """从循环输出数据生成 HTML 报告。如果 auto_refresh 为 True，则添加 meta 刷新标签。"""
    history = data.get("history", [])
    holdout = data.get("holdout", 0)
    title_prefix = html.escape(skill_name + " — ") if skill_name else ""

    # 获取训练集和测试集中所有唯一的查询（含 should_trigger 信息）
...
<body>
    ...
    <div class="explainer">
        <strong>正在优化你的技能描述。</strong> 此页面会自动更新，Claude 会测试技能描述的不同版本。每一行代表一次迭代——一次新的描述尝试。列代表测试查询：绿色勾表示技能正确触发（或正确不触发），红色叉表示判断错误。"训练"分数显示在用于改进描述的查询上的表现；"测试"分数显示在优化器未见过的保留查询上的表现。完成后，Claude 会将表现最好的描述应用到你的技能上。
    </div>
"""]
...
def main():
    ...
    parser.add_argument("--skill-name", default="", help="要包含在报告标题中的技能名称")
